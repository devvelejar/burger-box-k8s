...
resource "aws_api_gateway_rest_api" "burger-box" {
  name        = "burger-box"
  description = "API Gateway para autenticação via Cognito"
}

resource "aws_api_gateway_deployment" "deployment" {
  rest_api_id = aws_api_gateway_rest_api.burger-box.id
...
resource "aws_api_gateway_resource" "resource" {
  rest_api_id = aws_api_gateway_rest_api.burger-box.id
  parent_id   = aws_api_gateway_rest_api.burger-box.root_resource_id
  path_part   = "auth"
}
...
resource "aws_lambda_permission" "apigw_lambda" {
  ...
  source_arn    = "arn:aws:execute-api:us-east-1:467827799632:${aws_api_gateway_rest_api.burger-box.id}/*/*/auth"
}
